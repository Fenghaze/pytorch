...
def unpickle(file):
    with open(file, 'rb') as fo:
        dict_ = pickle.load(fo, encoding='bytes') # 反序列化对象，将文件中的数据解析为一个python对象
    return dict_
...
if __name__ == '__main__':
    if Train:
        for j in range(1, 6):
            ...
            train_data = unpickle(data_path) # 文件中的数据保存为字典形式
            print(data_path + " is loading...")

            for i in range(0, 10000):# 列出 10000 个样本的清单
                img = np.reshape(train_data[b'data'][i], (3, 32, 32)) # 把 目标维度转换为 3*32*32 的维度
                img = img.transpose(1, 2, 0) # 转置： 32*32*3

                label_num = str(train_data[b'labels'][i]) # 分类文件夹，按 label 分类
                o_dir = os.path.join(train_o_dir, label_num)
                my_mkdir(o_dir)

                img_name = label_num + '_' + str(i + (j - 1)*10000) + '.png'
                img_path = os.path.join(o_dir, img_name) # 图片的最终路径
                imsave(img_path, img)
